...
public class DownloadTheDataModel extends Observable {
	 ...
	 *
	 * @author Rokhaya
	 * the method connect handles the writing of hla files
	 * @param gui object
	...
	public void connect (GUI gui){
		...
		// The genename need to be changed because the string "-" is not accepted by SQL as table name
		int position = 0;
		String Genename = null;
		int chromosome = 6;
		int Genedistance = 0;
		
		// Initializing 2 array list to separate groups and subgroups 
		...
		try {
	      ...
	      // Initializing objects : Resultset and ResultMetaData which retrieve data from the database and allows to manipulated them
	      // This Resultset retrieve specifically data for ALL populations.
      	...
      	/* Different treatment are required depending on what is contained in populations
      	...
      	for(int p = 0; p < populations.size(); p++) {
      		    ...
      		    for(int i = 1; i+2 <= resultMeta.getColumnCount(); i=i+2){   
    	        	...
    	        	{ 
      					...
      					//pedwriter.print(result.getString("id")+ "\t" +  result.getString("id") + "\t" + Unknown + "\t"  + Unknown + "\t" + result.getString("gender") + "\t" + Unknown + "\t");
      	...
      	System.out.print("les groupes" + group);
			...
			if(group.isEmpty()== false && subgroup.isEmpty() == false) {
				System.out.print("Je rentre dans le 1er if");
				
				 for(int i = 1; i+5 <= resultMeta.getColumnCount(); i=i+2){
			...
			for(int g = 0; g < group.size(); g++) {
    		    ...
    		    if (resultgroup.next() == false)
    		    {
    		    	System.out.print("\n Il n'ya rien dans ton resultgroup");
    		    }
    		   
    		    while(resultgroup.next())
  					
	        	{ 
  					//pedwriter.print(resultgroup.getString("id")+ "\t" +  resultgroup.getString("id") + "\t" + Unknown + "\t"  + Unknown + "\t" + resultgroup.getString("gender") + "\t" + Unknown + "\t");
				...
				for(int sg = 0; sg < subgroup.size(); sg++) {
						...
						ResultSet resultsubgroup = conn.createStatement().executeQuery("SELECT * FROM "+Genename+",sbgroup  WHERE \"sbgroup\" = '"+subgroup.get(sg)+"' AND "+Genename+".id = sbgroup.id");
		    	        	...
		    	        	{ 
		      					//pedwriter.print(resultsubgroup.getString("id")+ "\t" +  resultsubgroup.getString("id") + "\t" + Unknown + "\t"  + Unknown + "\t" + resultsubgroup.getString("gender") + "\t" + Unknown + "\t");
			...
			System.out.print("\n je sors de mon 1er if");
					
					if(subgroup.isEmpty() && group.isEmpty() == false) {
						System.out.print("\n je rentre dans mon 2e if");
						
						  for(int i = 1; i+2 <= resultMeta.getColumnCount(); i=i+2){
							  System.out.print("je fais les fichiers du 1er if");
	      		          		infowriter.print(resultMeta.getColumnName(i) + "\t" + position  + "\n");
	      		          		mapwriter.print(chromosome + "\t" + resultMeta.getColumnName(i) + "\t" + Genedistance  + "\t" + position +  "\n");
	      		          	}
						
						for(int g = 0; g < group.size(); g++) {
							System.out.print("\n je suis dans le for group");
							System.out.print("group \t" + group.get(g));
		      		    ...
		      		    if (result2.next() == false)
		    		    {
		    		    	System.out.print("\n Il n'ya rien dans ton resultgroup");
		      				...
		      				while(result2.next()) 
		    	        	{ 
		      	    		    	System.out.print("\n Je suis dans le while de result2");
		      					
		      					//pedwriter.print(result2.getString("id")+ "\t" +  result2.getString("id") + "\t" + Unknown + "\t"  + Unknown + "\t" + result2.getString("gender") + "\t" + Unknown + "\t");
					...
					System.out.print("\n je sors de mon 2e if");
					
					if(group.isEmpty() && subgroup.isEmpty() == false) {
						System.out.print("\n je rentre dans mon 3e if");
						for(int i = 1; i+2 <= resultMeta.getColumnCount(); i=i+2){
							System.out.print("\n je rentre dans mon for .info");
      		          		infowriter.print(resultMeta.getColumnName(i) + "\t" + position  + "\n");
      		          		mapwriter.print(chromosome + "\t" + resultMeta.getColumnName(i) + "\t" + Genedistance  + "\t" + position +  "\n");
      		          	}
						for(int sg = 0; sg < subgroup.size(); sg++) {
							System.out.print("\n je rentre dans mon for subgroup");
						ResultSet result3 = conn.createStatement().executeQuery("SELECT * FROM "+Genename+",sbgroup  WHERE \"sbgroup\" = '"+subgroup.get(sg)+"' AND "+Genename+".id = sbgroup.id");
		      		      ResultSetMetaData resultMeta3 = result3.getMetaData();
		      				...
		      				while(result3.next()) 
		    	        	{ 
		      					System.out.print("\n je rentre dans le while subgroup");
		      					//pedwriter.print(result3.getString("id")+ "\t" +  result3.getString("id") + "\t" + Unknown + "\t"  + Unknown + "\t" + result3.getString("gender") + "\t" + Unknown + "\t");
